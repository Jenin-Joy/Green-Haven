...
def login(request):
     ...
     if request.method == "POST":
          ...
          deliverycount=tbl_delivery.objects.filter(delivery_email=email,delivery_password=password).count()
          if usercount > 0:
               ...
               delivery = tbl_delivery.objects.get(delivery_email=email,delivery_password=password)
               if(delivery.delivery_status==1):
                    request.session["deid"] = delivery.id
...
def delivery(request):
     dis = tbl_district.objects.all()
     if request.method == "POST":
          tbl_delivery.objects.create(delivery_name=request.POST.get("txtname"),
                                 delivery_email=request.POST.get("txtmail"),
                                   delivery_contact=request.POST.get("txtcontact"),
                                  delivery_address=request.POST.get("txtaddress"),
                                  delivery_photo=request.FILES.get("txtphoto"),
                                 delivery_proof =request.FILES.get("txtproof"),
                                   delivery_password =request.POST.get("txtpass"),
                                   place=tbl_place.objects.get(id=request.POST.get("Place")))
          return redirect("Guest:delivery")
     else:
